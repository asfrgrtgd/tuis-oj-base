...
## 2. ファイルパーミッションの設定（Linux/WSL2 の場合）
API/Worker コンテナは `appuser`（uid:65532）で動作するため、書き込みが発生するディレクトリのオーナーをホスト側で事前に設定します。
...
> **注**: macOS の Docker Desktop では不要です。Windows の場合は WSL2 内で実行してください。
